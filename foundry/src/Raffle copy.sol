...
contract Raffle is VRFConsumerBaseV2Plus, ReentrancyGuard, Pausable {
    ...
    function _processWinnerPayouts(uint256 raffleId, address winner) private {
        ...
        uint256 platformFee;
        if(raffle.featured){
            platformFee = (raffle.totalPrize * 20) / 100; // 20% for featured raffles
        } else {
            platformFee = (raffle.totalPrize * PLATFORM_FEE_PERCENTAGE) / 100; // 10% for non-featured raffles
        }
        uint256 winnerPrize = (raffle.totalPrize * WINNER_FEE_PERCENTAGE) / 100; // 10% for winner
        uint256 creatorPayout = raffle.totalPrize - platformFee - winnerPrize; // Remaining for creator
